...
public class BlueprintComponent : MonoBehaviour
{
    ...
    {
        sizeX = (int) cells.GetLength(0);
        sizeY = (int) cells.GetLength(1);
    ...
    private int findNext(int lastSubGrid, Vector2Int lastCell, out Vector2Int newCell, out Vector2 point) 
    {
        ...
        if (lastSubGrid == 4) 
        {
            if (lastCell.x - 1 >= 0 && cells[lastCell.x - 1, lastCell.y] != null) 
            {
        ...
        {
            if (lastCell.y - 1 >= 0 && cells[lastCell.x, lastCell.y - 1] != null) 
            {
        ...
        {
            if (lastCell.x + 1 < sizeX && cells[lastCell.x + 1, lastCell.y] != null) 
            {
                newCell.Set(lastCell.x + 1, lastCell.y);
                offsetX = -offsetX;
                next = 1;
        ...
        {
            if (lastCell.y + 1 < sizeY && cells[lastCell.x, lastCell.y + 1] != null) 
            {
                newCell.Set(lastCell.x, lastCell.y + 1);
                offsetX = -offsetX;
                offsetY = -offsetY;
    ...
    {
        if(x >= 0 && x < sizeX && y >= 0 && y < sizeY && cells[x, y] != null) 
        {
            if(cells[x, y].ApplyDamage(amount)) 
            {
                spawnDebris(x, y);
    ...
    private void spawnDebris(int x, int y) {
        ...
        debris.GetComponent<Renderer>().material.SetColor("_Color", cells[x, y].Color);
    }
    ...
    private void TryChunks(int x, int y)
    {
        ...
        { 
            ...
            foreach (var chunk in chunks)
            {
                ...
                chunkPrefab.cells = new Cell[chunkEndX - chunkStartX + 1, chunkEndY - chunkStartY + 1];

                foreach (var cell in chunk)
                {
                    chunkPrefab.cells[cell.X - chunkStartX, cell.Y - chunkStartY] = cell;
                    cells[cell.X, cell.Y] = null;
                } 
